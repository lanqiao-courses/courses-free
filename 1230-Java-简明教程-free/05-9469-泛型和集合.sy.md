...
泛型即参数化类型，指将数据类型定义为可变参数，相较于非泛型代码中固定的参数类型，泛型可根据程序需求动态指定类型。

定义泛型的核心规则：

1. 类型参数限制：泛型的类型参数只能是引用类型（如类、接口、数组），不支持基本数据类型（int、byte 等），需使用包装类（Integer、Byte 等）替代；
2. 多参数声明：可声明多个泛型参数，参数间用逗号分隔（如 `<K, V>`），支持多上界限定（`<T extends 类A & 接口B & 接口C>`，类需放在首位）；
3. 类型上下界：
   - 上界：通过 `<T extends 父类/接口>` 限定类型参数 T 的上界，T 只能是该父类 / 接口本身或其子类 / 实现类；也可通过通配符 `<? extends X>` 限定通配符的上界；
   - 下界：仅能通过通配符 `<? super 子类>` 限定下界，该通配符可接收的类型只能是该子类本身或其父类，**无法通过类型参数（如 K）直接声明 `<K super X>`**；
4. 通配符类型：支持无界通配符 `?`（表示任意引用类型，只读不可写，如 `Class<?>`），也可结合 `extends`/`super` 实现有界限制（`<? extends X>`/`<? super X>`）。
